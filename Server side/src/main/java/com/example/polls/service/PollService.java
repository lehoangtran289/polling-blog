...
public class PollService {
	/*
	 * Get the list of polls formatted in the form of PollResponse payloads that is
	...
	private void validatePageNumberAndSize(int page, int size) {
		if (page < 0)
			throw new BadRequestException("Page number cant < 0");

		if (size > AppConstants.MAX_PAGE_SIZE)
			throw new BadRequestException("Page size cant > " + AppConstants.MAX_PAGE_SIZE);
	}
	...
	Map<Long, User> getPollCreatorMap(List<Poll> polls) {
		...
		Map<Long, User> creatorMap = creators.stream().collect(Collectors.toMap(User::getId, Function.identity())); // users
		return creatorMap;
	}
	...
	public PollResponse castVoteAndGetUpdatePoll(Long pollId, @Valid VoteRequest voteRequest,
		...
		try {
			...
			logger.info("User {} jas already voted in Poll {}", currentUser.getId(), pollId);
			throw new BadRequestException("Sorry! You have already cast your vote in this poll");
		}

		// -- Vote saved, Return the updated Vote Response now --
